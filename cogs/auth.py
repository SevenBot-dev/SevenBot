...
class AuthCog(commands.Cog):
    ...
    async def on_raw_reaction_add(self, pl):
        ...
        if message.embeds[0].title == "認証ボタン" or message.embeds[
            ...
            try:
                r = guild.get_role(
                    int(
                        m0.description.splitlines()[1].split(": ")[1][
                            3:-1
                        ]
                    )
                )
            ...
            if message.embeds[0].title == "認証ボタン" or message.embeds[
                0
            ].title.endswith("リアクション"):
                if pl.emoji.name == "check5":
                    if r not in user.roles:
                        await user.add_roles(r)
                        try:
                            await user.send(
                                f"{guild.name} での認証が完了しました。"
                            )
                        except BaseException:
                            pass
            elif (
                message.embeds[0].title.endswith("画像認証")
                and r not in user.roles
            ):
                url, auth_text = await self.make_image_auth_url(
                    message
                )
                e = SEmbed(
                    color=Process,
                    title=get_txt(
                        message.guild.id, "img_auth_header"
                    ),
                    description=get_txt(
                        message.guild.id, "img_auth_desc"
                    )
                    + "\n"
                ...
                message.embeds[0].title.endswith("Web認証")
                and r not in user.roles
            ):
        ...
        if m0.title.startswith("認証ボタン - "):
            ...
            if m0.title.endswith("ワンクリック"):
                if r not in user.roles:
                    await user.add_roles(r)
                    await com.send(
                        f"{guild.name} での認証が完了しました。", hidden=True
                    )
                else:
                ...
                if r not in user.roles:
                    ...
                    try:
                        ...
                        await user.send(
                            get_txt(guild.id, "img_auth_ok").format(
                                com.channel.id
                            )
                        )
                        await user.add_roles(r)
                        ...
                        await com.send(
                            get_txt(guild.id, "timeout"), hidden=True
                        )
                else:
                    await com.send("すでに認証済みです。", hidden=True)
    ...
    async def auth_click(self, ctx, role: discord.Role = 0):
        ...
        if (
            ...
            e = discord.Embed(
                title=get_txt(ctx.guild.id, "no_role_perm").format(
                    role.name
                ),
                color=Error,
            )
