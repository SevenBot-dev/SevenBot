...
class AuthCog(commands.Cog):
    ...
    async def on_raw_reaction_add(self, pl):
        ...
        m0 = message.embeds[0]
        if message.embeds[0].title == "認証ボタン" or message.embeds[0].title.startswith(
            "認証ボタン - "
        ):
            ...
            try:
                ...
                r = guild.get_role(Guild_settings[pl.guild_id]["auth_role"])
            if message.embeds[0].title == "認証ボタン" or message.embeds[0].title.endswith(
                "リアクション"
            ):
                ...
                e = SEmbed(
                    color=Process,
                    title=get_txt(message.guild.id, "img_auth_header"),
                    description=get_txt(message.guild.id, "img_auth_desc")
                    + "\n"
                    + get_txt(message.guild.id, "img_auth_warn"),
                    image_url=url,
                )
                try:
                    ...
                    try:
                        ...
                        await msg.edit(
                            embed=discord.Embed(
                                title=get_txt(guild.id, "img_auth_ok").format(
                                    channel.id
                                ),
                                color=Success,
                            )
                        ...
                        await msg.edit(
                            embed=discord.Embed(
                                title=get_txt(guild.id, "timeout"), color=Error
                            )
                        )
                except Forbidden:
                    e = discord.Embed(title=get_txt(guild.id, "dm_fail"), color=Error)
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        "https://captcha.sevenbot.jp/session",
                        json={
                            "password": web_pass,
                            "uid": user.id,
                            "gid": guild.id,
    ...
    async def on_button_click(self, com):
        ...
        if m0.title.startswith("認証ボタン - "):
            ...
            try:
                ...
                r = guild.get_role(Guild_settings[com.guild.id]["auth_role"])
            if m0.title.endswith("ワンクリック"):
                ...
                if r not in user.roles:
                    ...
                    await com.send(
                        embed=SEmbed(
                            description=get_txt(com.guild.id, "img_auth_desc2")
                            + "\n"
                            + get_txt(com.guild.id, "img_auth_warn"),
                            image_url=url,
                            color=Process,
                        )
                    )
                    try:
                        ...
                        await user.send(
                            get_txt(guild.id, "img_auth_ok").format(com.channel.id)
                        )
                        await user.add_roles(r)
                    except asyncio.TimeoutError:
    ...
    async def auth_click(self, ctx, role: discord.Role = 0):
        ...
        if Guild_settings[ctx.guild.id]["auth_role"] == 0 and role == 0:
            e = discord.Embed(
                title="ロールが登録されていません", description="初回はロールを登録する必要があります", color=Error
            )
        ...
        if (
            ...
            e = discord.Embed(
                title=get_txt(ctx.guild.id, "no_role_perm").format(role.name),
                color=Error,
            )
            await ctx.reply(embed=e)
            return
            ...
            e = discord.Embed(
                title=get_txt(ctx.guild.id, "no_role_perm_bot").format(role.name),
                color=Error,
            )
            await ctx.reply(embed=e)
            return
        ...
        e = discord.Embed(
            title="認証ボタン - ワンクリック",
            description=f'下のボタンを押して認証\nロール: {ctx.guild.get_role(Guild_settings[ctx.guild.id]["auth_role"]).mention}',
            color=Widget,
        )
        await components.send(
            ctx, embed=e, components=[components.Button("認証", "auth")]
        )
        try:
    ...
    async def make_image_auth_url(self, message):
        ...
        auth_text = hashlib.md5(str(time.time()).encode()).hexdigest()[0:8]
        w, h = draw.textsize(auth_text, font=bfnt)
        draw.text(((640 - w) / 2, (640 - h) / 2), auth_text, fill="white", font=bfnt)
