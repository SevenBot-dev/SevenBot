...
class AuthCog(commands.Cog):
    ...
    async def on_raw_reaction_add(self, pl):
        ...
        if message.embeds[0].title == "認証ボタン" or message.embeds[0].title.startswith("認証ボタン - "):
            ...
            user = guild.get_member(pl.user_id)
            try:
                r = guild.get_role(int(m0.description.splitlines()[1].split(": ")[1][3:-1]))
            except IndexError:
                r = guild.get_role(self.bot.guild_settings[pl.guild_id]["auth_role"])
            if message.embeds[0].title == "認証ボタン" or message.embeds[0].title.endswith("リアクション"):
                ...
                try:
                    ...
                    try:
                        ...
                        await msg.edit(
                            embed=discord.Embed(
                                title=get_txt(guild.id, "img_auth_ok").format(channel.id),
                                color=Success,
                            )
                        )
                        await user.add_roles(r)
                        ...
                        await msg.edit(
                            embed=discord.Embed(
                                title=get_txt(guild.id, "timeout"),
                                color=Error,
                            )
                        )
                except Forbidden:
                    e = discord.Embed(
                        title=get_txt(guild.id, "dm_fail"),
                        color=Error,
                    )
    ...
    async def on_button_click(self, com):
        ...
        if m0.title.startswith("認証ボタン - "):
            ...
            try:
                ...
                r = guild.get_role(self.bot.guild_settings[com.guild.id]["auth_role"])
    ...
    async def auth_click(self, ctx, role: discord.Role = 0):
        if self.bot.guild_settings[ctx.guild.id]["auth_role"] == 0 and role == 0:
            e = discord.Embed(
                title="ロールが登録されていません",
            ...
            return
        if role == 0:
            role = ctx.guild.get_role(self.bot.guild_settings[ctx.guild.id]["auth_role"])
        ...
        role_mention = ctx.guild.get_role(self.bot.guild_settings[ctx.guild.id]["auth_role"]).mention
        e = discord.Embed(
            title="認証ボタン - ワンクリック",
