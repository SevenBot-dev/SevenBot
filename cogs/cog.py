...
from discord import CategoryChannel, DMChannel, Forbidden, NotFound
...
class MainCog(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if isinstance(message.channel, DMChannel):
            """if is_command(message):
                            c=message.content[3:]
                            if c == "sevennet_dm":
                                            e=discord.Embed(title="通知設定",description=f"{Number_emojis[1]}レベルアップ")
                                            await message.author.send(embed=e)"""
            if message.content == "level":
                if message.author.id in Levelup_off:
                    Levelup_off.remove(message.author.id)
                else:
                    Levelup_off.append(message.author.id)
                e = discord.Embed(title="通知設定", description="レベルアップ通知は" + (
                    "OFF" if message.author.id in Levelup_off else "ON") + "になりました。", color=Success)
                await message.author.send(embed=e)
            return
        ...
        if Guild_settings[message.guild.id]["level_active"] and message.channel.id not in Guild_settings[message.guild.id]["level_ignore_channel"]:
            ...
            if Guild_settings[message.guild.id]["levels"][message.author.id] * 10 <= Guild_settings[message.guild.id]["level_counts"][message.author.id]:
                ...
                if message.author.id not in Levelup_off:
                    try:
                        await message.author.send(embed=e)
    ...
    async def about(self, ctx):
        ...
        e.add_field(name=get_txt(ctx.guild.id, "abouts")[10],
                    value='[sevenc-nanashi/SevenBot-Issues](https://github.com/sevenc-nanashi/SevenBot-Issues)')
        e.add_field(name=get_txt(ctx.guild.id, "abouts")
