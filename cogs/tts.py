...
def only_premium():
    async def predicate(ctx):
        ...
        if res:
            return True
        e = discord.Embed(
            title=get_txt(ctx.guild.id, "premium"),
            description=get_txt(ctx.guild.id, "premium_desc"),
            color=Premium_color,
        )
...
Tts_default = {"qu": [], "playing": False, "called_channel": 0, "last_speaker": 0}
...
class TtsCog(commands.Cog):
    ...
    async def tts_join(self, ctx):
        if ctx.author.voice is None:
            e = discord.Embed(
                title=get_txt(ctx.guild.id, "tts_vc_none"),
                description=get_txt(ctx.guild.id, "tts_vc_none_desc"),
                color=Error,
            )
            await ctx.reply(embed=e)
            return
    ...
    async def on_message(self, message):
        ...
        if (
            Tts_channels[message.guild.voice_client.channel.id].get("last_speaker", 0)
            != message.author.id
        ):
            flag = True
    ...
    async def auto_left(self, member, before, after):
        if before.channel is None:
            ...
            return
        if (
            not [m for m in before.channel.members if not m.bot]
        ) and before.channel.guild.voice_client.channel == before.channel:
    ...
    async def tts_dicts_remove(self, ctx, *, txt):
        ...
        if txt in Guild_settings[ctx.guild.id]["tts_dicts"].keys():
            res += "`" + Guild_settings[ctx.guild.id]["tts_dicts"][txt][1] + "`\n"
        ...
        if count == 0:
            e = discord.Embed(
                title=get_txt(ctx.guild.id, "tts_dicts_rem_fail").format(txt),
                description=get_txt(ctx.guild.id, "tts_dicts_rem_fail_desc"),
                color=Error,
            )
        else:
            e = discord.Embed(
    ...
    async def tts_dicts_list(self, ctx):
        ...
        if gs["tts_dicts"] == {}:
            e = discord.Embed(
                title="登録されていません。",
                description="`sb#tts dicts add`で登録してください。",
                color=Error,
            )
            await ctx.reply(embed=e)
        else:
    ...
    async def play_voice(self, txt, message):
        ...
        htsvoice = ["-m", f'./htsvoices/{ts.get("speaker", user.id % 4)}.htsvoice']
        ...
        try:
            ...
            c.terminate()
        loop.create_task(
            message.remove_reaction(Official_emojis["network"], message.guild.me)
        )
        # await c.wait()
        ...
        if not Tts_channels[guild.voice_client.channel.id]["playing"]:
            ...
            try:
                ...
                while Tts_channels[guild.voice_client.channel.id]["qu"]:
                    ...
                    if message:
                        loop.create_task(rm.add_reaction(Official_emojis["voice"]))
                        loop.create_task(
                            rm.remove_reaction(Official_emojis["queue"], self.bot.user)
                        )
                    #                     txt=q.clean_content
                    ...
                    if message:
                        loop.create_task(
                            rm.remove_reaction(Official_emojis["voice"], self.bot.user)
                        )
                        loop.create_task(rm.add_reaction(Official_emojis["check8"]))
    ...
    async def save_tts_setting(self, member):
        await self.bot.db.user_settings.update_one({"uid": member.id}, {"$set": {"tts_settings": Tts_settings[member.id]}})
